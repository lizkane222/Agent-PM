...
/**
 * Corner marker shown on every action item card that is pinned to Focus.
 *
 * Always top-right — the position is deliberately consistent across all card variants
 * (Action Items, Calendar, Account Detail). The card root must be `position: relative`.
 * `className` overrides positioning only; prefer not to use it.
 */
...
/**
 * The "Pin to Focus" / "Unpin from Focus" context-menu entry.
 *
 * Shared so every action item card offers the same label and icon. Callers should skip
 * this entry entirely for `local-*` blank cards — see hooks/useFocusPins.
 */
export function focusPinMenuItem(isPinned: boolean, onToggle: () => void): ContextMenuItem {
  return {
    label: isPinned ? "Unpin from Focus" : "Pin to Focus",
    icon: <svg viewBox="0 0 16 16" fill="currentColor" className="w-3.5 h-3.5"><path d={PIN_PATH} /></svg>,
    onClick: onToggle,
  };
}
