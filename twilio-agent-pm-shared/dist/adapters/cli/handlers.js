...
import { z } from "zod";
import { getTitleRole, dueDateGroup, getRsvp, fileIcon, attachLinkIcon, fmtBytes, fmtTime, formatArr, calendarEventDisplayProps, matchResourceLabel, isTokenExpired, toggleExportItem, findNode, removeNode, canvasReducer, INITIAL_HISTORY, claudeSkillTransition, agentSkillTransition, } from "../../brain/index.js";
import { AirtableActionItemSchema, CalendarEventSchema, ExportItemSchema, CanvasNodeSchema, } from "../../schemas.js";
...
export function dispatch(envelope) {
    ...
    switch (command) {
        ...
        case "getRsvp": {
            ...
            return { rsvp: getRsvp({ attendees: attendees }, userEmail) };
        }
        case "fileIcon": {
        ...
        case "toggleExportItem": {
            ...
            return { items: toggleExportItem(items, item) };
        }
        case "canvasFind": {
            const { root, id } = CanvasFindPayload.parse(payload);
            return { node: findNode(root, id) };
        }
        case "canvasRemove": {
            const { root, id } = CanvasRemovePayload.parse(payload);
            return { root: removeNode(root, id) };
        }
        case "canvasReducer": {
            const { state, action } = CanvasReducerPayload.parse(payload);
            const s = (state ?? INITIAL_HISTORY);
            return canvasReducer(s, action);
        }
