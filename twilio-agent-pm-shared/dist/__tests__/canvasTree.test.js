...
import { FIND_NODE_FIXTURES, REMOVE_NODE_FIXTURES, DEEP_CLONE_FIXTURES, RICH_TEXT_PROPS_FIXTURES, } from "../fixtures/canvasTree.fixtures.js";
let idCounter = 0;
    ...
    for (const f of DEEP_CLONE_FIXTURES) {
        it(f.note ?? `clone type="${f.source.type}"`, () => {
            ...
            if (f.offsetRoot && f.source.props["x"] !== undefined) {
                expect(clone.props["x"]).toBe(f.source.props["x"] + 20);
                expect(clone.props["y"]).toBe((f.source.props["y"] ?? 0) + 20);
            }
        });
    }
});
...
//# sourceMappingURL=canvasTree.test.js.map