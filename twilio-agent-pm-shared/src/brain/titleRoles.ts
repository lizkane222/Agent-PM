...
export const ROLE_ORDER: TitleRole[] = ["SA", "PM", "PgM", "CSM", "MA", "TAM", "AE", "ENG", "other"];

export const ROLE_META: Record<TitleRole, { border: string; bg: string; text: string; label: string; slug: string }> = {
...
export const ROLED_PAGES: TitleRole[] = ["SA", "CSM", "PM", "PgM", "MA", "TAM", "AE"];

export const SLUG_TO_ROLE: Record<string, TitleRole> = Object.fromEntries(
  (Object.entries(ROLE_META) as [TitleRole, (typeof ROLE_META)[TitleRole]][]).map(([k, v]) => [v.slug, k])
) as Record<string, TitleRole>;

// Match in specificity order: TAM before MA, SA before generic.
export function getTitleRole(title: string | null | undefined): TitleRole {
  if (!title) return "other";
  const t = title.toLowerCase();
  if (t.includes("solution") && t.includes("architect")) return "SA";
  if (/\bsa\b/.test(t)) return "SA";
  if (t.includes("technical account")) return "TAM";
  if (/\btam\b/.test(t)) return "TAM";
  if (t.includes("customer success")) return "CSM";
  if (/\bcsm\b/.test(t)) return "CSM";
  if (t.includes("program manager") || t.includes("project manager")) return "PgM";
  if (/\bpgm\b/.test(t)) return "PgM";
  if (t.includes("product manager")) return "PM";
  if (/\bpm\b/.test(t)) return "PM";
  if (t.includes("manager")) return "MA";
  if (/\bma\b/.test(t)) return "MA";
  if (t.includes("account executive") || /\bae\b/.test(t)) return "AE";
  if (t.includes("engineer") || /\beng\b/.test(t)) return "ENG";
  return "other";
}
